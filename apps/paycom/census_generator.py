...
def render_auto_fix_options(key_prefix):
    ...
    with col_fix1:
        ...
        fix_driver_smart = st.checkbox("Enable Smart Driver Correction (Dept/Job -> FLSA)", value=False, key=f"{key_prefix}_fix_driver_smart", help="First fills blank Position from 'Driver' departments, then automatically sets BLANK FLSA to Non-Exempt for all Drivers. It will NOT replace existing FLSA values.")
