...
def render_census_sanity_check():
    ...
    if len(missing_critical) > 1:
        st.error("⚠️ **Wrong file format** — this doesn't look like a Paycom Census export. If you meant to upload an ADP file, switch to the ADP section in the sidebar.")
        return
...
| **Dates** | All dates formatted as MM/DD/YYYY |
    ...
    default_work_loc = st.text_input("📍 Default Work Location for Blanks", placeholder="Type to fill all blank work locations...", key="pc_sanity_default_loc_input")
    st.markdown("---")
    ...
    if st.session_state.get(show_key):
        ...
        if data_key not in st.session_state:
            with st.spinner("Preparing downloads..."):
                ...
                if src_loc_col and src_loc_col in df_download.columns:
                    ...
                    dwl = st.session_state.get("pc_sanity_default_loc_input", "").strip()
                    if dwl:
                    ...
                    def _map_loc(x):
                        if pd.isna(x) or str(x).lower().strip() == 'nan' or str(x).strip() == "": return ""
                        curr = str(x).strip()
                        return loc_dict.get(curr, curr)
                    df_download[src_loc_col] = df_download[src_loc_col].apply(_map_loc)

                date_cols = [resolved_field_map.get('Hire Date'), resolved_field_map.get('Original Hire Date'), resolved_field_map.get('Termination Date'), resolved_field_map.get('DOB'), resolved_field_map.get('License Expiration Date')]
        ...
        with col_dl1:
            st.download_button(label="📥 Download Corrected Source (XLSX)", data=cached.get("xlsx", b""), file_name=f"Paycom_Cleaned_{stamp}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="pc_sanity_dl_xlsx")
        with col_dl2:
            st.download_button(label="📥 Download Corrected Source (CSV)", data=cached.get("csv", b""), file_name=f"Paycom_Cleaned_{stamp}.csv", mime="text/csv", key="pc_sanity_dl_csv")
        with col_dl3:
