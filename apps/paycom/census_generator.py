        ...
        with st.expander("🛠️ Census Integrity & Mapping Action Center", expanded=not hard_errors.empty):
            ...
            with wcol1:
                ...
                if not smart_driver_fixes.empty: 
                    st.markdown(f"- 🚛 **Smart Driver Fixes:** {len(smart_driver_fixes)} employee(s).{get_ids_str(smart_driver_fixes)}")
            with wcol2:
                ...
                if not position_blanks.empty: 
                    st.markdown(f"- ℹ️ **Position Auto-Fill:** {len(position_blanks)} employee(s).{get_ids_str(position_blanks)}")
    else:
