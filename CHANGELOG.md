...
- **A big census that outlasts the request is no longer lost.** The push waits 120 s, and a gateway can give up sooner (502/503/504), while Uzio keeps processing — run 1376 (873 employees) took 8 minutes. The answer, and with it `OnboardingAutomationId`, never arrives. The tool now finds the run itself: the FIRST run this user started for this FEIN since the push began (60 s allowance for clock drift). First rather than newest, because a later run by the same user must not be taken for it — a newest-first lookup for run 1376 returned the Payment run 1377 that followed it. The row exists from the moment processing starts, so a still-running census shows "still processing — started HH:MM IST, N min ago", and **Check the run log again** fills in the errors once it finishes. If no run is found, the tool says the census most likely never reached Uzio. FEIN and username are checked against a strict character set before they go into the query.
